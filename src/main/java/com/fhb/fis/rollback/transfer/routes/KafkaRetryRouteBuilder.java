...
public class KafkaRetryRouteBuilder extends OABServiceRouteBuilder{
    ...
    public void configureEntryRoute(RouteDefinition fromEntry) {
        ...
        .end()
        .log(LoggingLevel.INFO, LOGGER, "Initializing retryroutebuilder body:${body}")
        .choice()
        //.process(EnvelopeWrapperProcessor)
        .when().simple("${header["+KafkaConstants.RETRIES_HEADER+"]} < {{kafka.retries}}")
            .log(LoggingLevel.INFO, LOGGER, "Retries not exceeded retries ${header["+KafkaConstants.RETRIES_HEADER+"]}")
            .setHeader(KafkaConstants.RETRIES_HEADER).exchangeProperty(KafkaConstants.RETRIES_HEADER)
